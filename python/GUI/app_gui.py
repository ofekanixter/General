...
NEXTPAGE=False
def make_window(theme,nextPage=False):
    sg.theme(theme)
    ...
    data = [["John", 10], ["Jen", 5]]
    headings = ["Name", "Score"]
    input_layout_file =[[sg.Text('mp4 format? '),sg.Checkbox('Checkbox', default=False, k='-MP4_FILE-')],
                    [sg.Text('input file')],
                    [sg.Combo(sg.user_settings_get_entry('-filenames-', []),
                        default_value="C:/Users/SmadarENB3/Desktop/ofek/programing/python/titles.txt",
                        size=(50, 1), key='-FILE_FILE-'),sg.FileBrowse()],
                    [sg.Text('output save folder')],
                    [sg.Combo(sg.user_settings_get_entry('-foldername-', []),
                        default_value="C:/Users/SmadarENB3/Desktop/songs",
                        size=(50, 1), key='-FN_FILE-'),sg.FolderBrowse()],
                    [sg.Button('Submit',k='Submit_File'), sg.Button('Popup')]
                    ]
    ...
    input_layout_text =  [
                    [sg.Text('mp4 format? '),sg.Checkbox('Checkbox', default=False, k='-MP4_TEXT-')],
                    [sg.Text('output save folder')],
                    [sg.Combo(sg.user_settings_get_entry('-foldername-', []),
                        default_value="C:/Users/SmadarENB3/Desktop/songs",
                        size=(50, 1), key='-FN_TEXT-'),sg.FolderBrowse()],
                    [sg.Text('text')],
                    [sg.Multiline(''
                                    , size=(20,5), expand_x=True, expand_y=True, k='-MLINE-')],
                    [sg.Button('Submit',k='Submit_Text'), sg.Button('Popup')]
                    ]
                
    input_layout_audio = [[sg.T('merge all song in a folder to one song')],
                    [sg.Text('input folder')],
                    [sg.Combo(sg.user_settings_get_entry('-foldername-', []),
                        default_value="C:/Users/SmadarENB3/Desktop/songs/concat",
                        size=(50, 1), key='-FN_IN_AUDIO-'),sg.FolderBrowse()],
                    [sg.Text('output folder')],
                    [sg.Combo(sg.user_settings_get_entry('-foldername-', []),
                        default_value="C:/Users/SmadarENB3/Desktop/songs/concat/merged",
                        size=(50, 1), key='-FN_OUT_AUDIO-'),sg.FolderBrowse()],
                    [sg.Text('new name')],

                    [sg.Button('Submit',k='Submit_Audio'), sg.Button('Popup')]]
    ...
    layout = [ [sg.MenubarCustom(menu_def, key='-MENU-', font='Courier 15', tearoff=True)],
                [sg.Text('Demo Of (Almost) All Elements', size=(38, 1),
                          justification='center', font=("Helvetica", 16), relief=sg.RELIEF_RIDGE,
                            k='-TEXT HEADING-', enable_events=True)]]
    layout +=[[sg.TabGroup([[  sg.Tab('Text File', input_layout_file),
                               sg.Tab('Free Text', input_layout_text),
                               sg.Tab('Audio', input_layout_audio),
                               sg.Tab('PDF', graphing_layout),
...
def main():
    ...
    while True:
        ...
        if event in (None, 'Exit'):
            ...
            print(values["-TAB GROUP-"]=="Text File")
            print("[LOG] Clicked Submit!")
            file = values['-FILE_FILE-']
            folder = values['-FN_FILE-']
            MP4_format = values['-MP4_FILE-']
            sg.popup("You chose: " + file+"\nYou chose: " 
                     + folder+"\nYou chose: " + str(MP4_format)+
                     "\nYou chose: ", keep_on_top=True)
            print("[LOG] User chose file: " + str(file))
            ms.start("FILE",file , folder, MP4_format)
            print(file , folder, MP4_format)
            #NEXTPAGE=True
            #make_window(sg.theme(),NEXTPAGE)
        elif event == "Submit_Text":
            print(values["-TAB GROUP-"])
            print(values["-TAB GROUP-"]=="Free Text")
            print("[LOG] Clicked Submit!")
            folder = values['-FN_TEXT-']
            text = values['-MLINE-']
            MP4_format = values['-MP4_TEXT-']
            sg.popup("\nYou chose: " 
                     + folder+"\nYou chose: " + str(MP4_format)+
                     "\nYou chose: " + text, keep_on_top=True)
            ms.start("TEXT",text , folder, MP4_format)
            print(text , folder, MP4_format)
            #NEXTPAGE=True
            #make_window(sg.theme(),NEXTPAGE)
        elif event == "Submit_Audio":
            print(values["-TAB GROUP-"])
            print(values["-TAB GROUP-"]=="Free Text")
            print("[LOG] Clicked Submit!")
            folder_in = values['-FN_IN_AUDIO-']
            folder_out = values['-FN_OUT_AUDIO-']
            sg.popup("\nYou chose: " 
                     + folder_in+"\nYou chose: " + folder_out, keep_on_top=True)
            ms.start("AUDIO",folder_in , folder_out)
